"""
leak_mlpBF.py -- FULL DATASET (all 36 EEGMAT subjects), relaxed vs. one
mental-arithmetic stress state ("before count" = relaxed vs. "during
count" = focus/stress).

Data-handling condition: DATA-LEAK. The train/test split (train_test_split,
stratified, epoch-level) has no notion of subject identity, so 4-second
windows from the same subject can land on both sides of the split. Reported
accuracy therefore reflects some amount of within-subject pattern recall in
addition to genuine relax-vs-stress signal. Compare against
../no_leak_shap/mshap_mlpBF.py, which uses a GroupShuffleSplit keyed on
subject to remove this channel.
