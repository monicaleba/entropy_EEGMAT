"""
mshap_mlpBCC.py -- BAD-COUNTER subjects only (EEGMAT), relaxed vs. one
mental-arithmetic stress state.

Data-handling condition: NON-LEAK. GroupShuffleSplit keeps every epoch from
one EDF file (one subject) entirely on one side of the train/test split, so
no subject appears in both -- removing the leakage channel present in
../leak/leak_mlpBBC.py. Also runs a post-hoc SHAP explainability pass on
the trained model (see run_shap_analysis()).

NOTE ON NAMING: this subset is "BCC" here (data_BCC) but "BBC" in the
data-leak counterpart (leak_mlpBBC.py, data_BBC) -- both mean Bad-Counter.

Expects ./data_BCC/relaxed/*.edf and ./data_BCC/non-relaxed/*.edf.
Outputs: ./results_BCC/{model.pth, history.npy, confusion_matrix.png,
metrics_per_epoch.png} and ./shap_outputs_BCC/{shap_beeswarm.png,
...
BATCH_SIZE = 16
NUM_EPOCHS = 1000
RESULTS_DIR = "results_BCC"
os.makedirs(RESULTS_DIR, exist_ok=True)

# ---------------------------------------------------------------------
# SHAP settings
# ---------------------------------------------------------------------
SHAP_OUTPUT_DIR = "shap_outputs_BCC"
