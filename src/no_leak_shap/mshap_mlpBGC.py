"""
mshap_mlpBGC.py -- GOOD-COUNTER subjects only (EEGMAT), relaxed vs. one
mental-arithmetic stress state.

Data-handling condition: NON-LEAK -- see mshap_mlpBCC.py for details.
Same pipeline as mshap_mlpBCC.py, pointed at the Good-Counter subset.

Expects ./data_BGC/relaxed/*.edf and ./data_BGC/non-relaxed/*.edf.
Outputs: ./results_BGC/{model.pth, history.npy, confusion_matrix.png,
metrics_per_epoch.png} and ./shap_outputs_BGC/{shap_beeswarm.png,
shap_feature_importance.{png,csv}, shap_channel_importance.{png,csv},
shap_values_raw.csv}.

Fix applied vs. the original file: the console banner before the final
metrics printed "BAD COUNT" (copy-pasted from mshap_mlpBCC.py) -- corrected
to "GOOD COUNT" below; this only affects a printed log line, not any
computed value.
"""
import os
import re
...
BATCH_SIZE = 16
NUM_EPOCHS = 1000
RESULTS_DIR = "results_BGC"
os.makedirs(RESULTS_DIR, exist_ok=True)

# ---------------------------------------------------------------------
# SHAP settings
# ---------------------------------------------------------------------
SHAP_OUTPUT_DIR = "shap_outputs_BGC"
